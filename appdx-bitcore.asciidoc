...
((("Bitcore", id="bitcore16")))Bitcore is a suite of tools provided by BitPay. Its goal is to provide easy-to-use tools for Sumcoin developers. Almost all of Bitcore's code is written in JavaScript. There are some modules written specifically for NodeJS. Finally, the "node" module of Bitcore includes Sumcoin Core's C++ code. Please see https://bitcore.io for more information.

=== Bitcore's Feature List

* Sumcoin full node (bitcore-node)
* Block explorer (insight)
* Block, transaction, and wallet utilities (bitcore-lib)
* Communicating directly with Sumcoin's P2P network (bitcore-p2p)
...
* Integrating services directly with Sumcoin Core (bitcore-node)
...
> utxo = {
  ...
  sumtoshis: amount sent to the address
}
...
Broadcasting a transaction to the Sumcoin network
